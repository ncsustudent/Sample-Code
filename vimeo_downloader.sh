...
if [ $# -ne 1 ]; then
	...
	echo "    with improvements by Jori Hamalainen"
	echo
...
# Get the main page
VIDEO_XML=`wget --load-cookies=brownies.txt -U \"${USER_AGENT}\" -q -O - http://vimeo.com/${VIMEO_ID}`

# Get the config url
CONFIG_URL=`echo $VIDEO_XML | grep data-config-url | perl -p -e 's/^.*? data-config-url="(.*?)".*$/$1/g' | perl -pe 's/&amp;/&/g'`
VIDEO_CONFIG=`wget --load-cookies=brownies.txt -U \"${USER_AGENT}\" -q -O - ${CONFIG_URL}`
...
echo \"${USER_AGENT}\"
 echo "!!!!!!!!!!!!!!!!!!!!"
# Do the download
wget -U \"${USER_AGENT}\" --load-cookies=brownies.txt -O ${FILENAME} ${DOWNLOAD_URL}
